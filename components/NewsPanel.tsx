        ...
        {MOCK_NEWS_EVENTS.map(event => (
          <div key={event.id} className="bg-[#161a1e] border border-white/5 rounded-[32px] p-6 flex flex-col md:flex-row justify-between items-center gap-6 group hover:border-blue-500/20 transition-all">
            <div className="flex items-center gap-6 flex-1">
               <div className={`size-14 rounded-[20px] flex items-center justify-center font-black text-xs border ${event.impact === 'HIGH' ? 'bg-red-500/10 border-red-500/20 text-red-500' : 'bg-yellow-500/10 border-yellow-500/20 text-yellow-500'}`}>
                  {event.impact === 'HIGH' ? '!!!' : '!!'}
               </div>
               <div className="space-y-1">
            ...
            <div className="flex gap-4 w-full md:w-auto">
              ...
              {event.impact === 'HIGH' && (
                <div className="flex items-center justify-center px-4 bg-red-500/10 rounded-2xl border border-red-500/20">
